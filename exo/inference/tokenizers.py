...
async def _resolve_tokenizer(model_id_or_local_path: Union[str, PathLike]):
  ...
  try:
    ...
    processor = AutoProcessor.from_pretrained(model_id_or_local_path, use_fast=True if "Mistral-Large" in f"{model_id_or_local_path}" else False, trust_remote_code=True,session=session)
